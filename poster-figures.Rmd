...
plot_df <- jop %>% 
  select(sigma, prob_end_mean, prob_end_25, prob_end_75, 
         prob_out10_mean, prob_out10_25, prob_out10_75,
  ...
  select(-name) %>%
  mutate(across(output, ~factor(., levels = c("(a) Probability disease is endemic", "(b) Probability of an outbreak > 10 hosts","(c) Peak number of cases")))) %>% 
...
disc_sens_df <- rbind(mutate(jopab, label = "a", 
                             long_label = as.character(unname(TeX("(a) $\\alpha_{\\beta}$ on")))),
                        ...
                        mutate(jopabt, label = "d",
                               long_label = as.character(unname(TeX("(d) $\\alpha_{\\beta}$ and $\\alpha_{\\tau}$ on")))),
                        mutate(jopab, label = "e",
                               long_label = as.character(unname(TeX("(e) $\\alpha_{\\beta}$ and $\\alpha_{\\mu}$ on")))),
                        mutate(jopab, label = "f",
  ...
  select(-name) %>% 
  # mutate(across(label, ~factor(., levels = c("(a) $\\alpha_{\\beta}$ on",
  #                       "(b) $\\alpha_{\\tau}$ on",
  #                       "(c) $\\alpha_{\\mu}$ on",
  #                       "(d) $\\alpha_{\\beta}$ and $\\alpha_{\\tau}$ on",
  #                       "(e) $\\alpha_{\\beta}$ and $\\alpha_{\\mu}$ on",
  #                       "(f) $\\alpha_{\\tau}$ and $\\alpha_{\\mu}$ on"
  # )))) %>% 
  pivot_wider(id_cols = c(sigma, R0, label, long_label), names_from = type) %>% 
  arrange(sigma, R0)

strip_labels <- list(TeX("(a) $\\alpha_{\\beta}$ on"),
                       TeX("(b) $\\alpha_{\\tau}$ on"),
                        TeX("(c) $\\alpha_{\\mu}$ on"),
                        TeX("(d) $\\alpha_{\\beta}$ and $\\alpha_{\\tau}$ on"),
                        TeX("(e) $\\alpha_{\\beta}$ and $\\alpha_{\\mu}$ on"),
...
ribbon_df <- right_join(filter(new_sens, variable == "fake") %>% mutate(min = -abs(value)) %>%  select(-value),
                   filter(new_sens, variable == "fake") %>% mutate(max = abs(value)) %>% select(-value))

new_sens_plot <- filter(new_sens, variable != "fake") %>% 
