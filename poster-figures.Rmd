...
R0_cols = c("#3A549F", "#5487BD", "#F3E3AB", "#FDC274", "#F89355", "#E45036", "#B00C27")
```
...
ggplot(jop, aes(x = sigma, y = prob_end_mean)) +
  ...
  scale_color_manual(values = R0_cols) +
  scale_fill_manual(values = R0_cols) + #c("#4063A8", "#BDE1ED", "#EAECCC", "#FDD687", "#FAA35D", "#EC643F", "#C01E29")) +
  theme_cowplot() +
...
ggplot(jop, aes(x = sigma, y = prob_out10_mean)) +
  ...
  # scale_fill_viridis(discrete=TRUE)+
  scale_color_manual(values = R0_cols) +
  scale_fill_manual(values = R0_cols) +
  theme_cowplot() +
  labs(color = unname(TeX("$R_0$")), fill = unname(TeX("$R_0$"))) +
