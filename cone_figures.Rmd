...
# run "\code\julia\deterministic_results.jl" to get this file
trajectories <- read_csv("results/illustrate_trajectories_proportionalSDE.csv", 
                         show_col_types = FALSE) #%>% 
#   # ungroup() %>%
#   filter(t %in% 1:(11*365)) %>%
#   filter(H >= 0, V >= 0) %>%
...
R0s_to_plot =  unique(trajectories$R0) #c(0.75, 0.95, 1.05, 1.25, 2)
sigmas_to_plot = unique(trajectories$sigma) #c(0.0, 0.05, 0.1, 0.15, 0.2, 0.25)
...
quadrant_plot <- quadrant_df %>%
  ...
  scale_color_manual(values = c("#61D04F", "#DF536B"), breaks = c("FALSE", "TRUE")) + #c("seagreen3", "plum4")
...
ggsave("figures/example_quadrants.png", quadrant_plot, height = 30, width = 30)
