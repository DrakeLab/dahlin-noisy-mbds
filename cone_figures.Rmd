...
trajectories <- read_csv("results/illustrate_trajectories_proportionalSDE.csv", show_col_types = FALSE) %>% # run "\code\julia\deterministic_results.jl" to get this file
  ungroup() %>%
  filter(t %in% 1:(11*365)) %>%quad
  filter(H >= 0, V >= 0) %>%
  group_by(trajectory, sigma, R0) # %>%
  # mutate(
  #   rolling_H = rollmean(H, roll_amount, mean, align = "right", fill = 0),
  #   rolling_V = rollmean(V, roll_amount, mean, align = "right", fill = 0)) #%>% 
# mutate(t = t - 365) %>% filter(t > -1)
...
end_stats <- trajectories %>% 
  ...
  # group_by(sigma, R0) %>%
  # summarise(num_end = sum(end_bool), .groups = "keep")

trajectories <- left_join(trajectories, end_stats) %>% 
  mutate(type = "stochastic") %>% 
  mutate(end_bool = replace_na(end_bool, FALSE))

saveRDS(trajectories, "results/formatted_trajectories.rds")

# Load deterministic results and repeat them for each value of sigma (for plotting)
deterministic_results <- read_csv("results/deterministic_trajectories.csv", show_col_types = FALSE) %>%
...
R0s_to_plot = c(0.75, 0.95, 1.05, 1.25, 2) #unique(trajectories$R0) #
sigmas_to_plot = c(0.0, 0.05, 0.1, 0.15, 0.2, 0.25) #unique(trajectories$sigma) #

trajectories <- trajectories %>% 
  filter(R0 %in% R0s_to_plot,
         sigma %in% sigmas_to_plot)
...
#                          levels = c("6.5", "4", "2", "1.25", "1.05", "0.95"))
# # levels = c("0.95", "1.05", "1.25","2"))

summary_df <- quadrant_df  %>% 
  select(sigma, R0, trajectory, end_bool) %>% 
  distinct() %>% 
  group_by(sigma, R0) %>% 
  summarize(num_died = 100 - sum(end_bool)) %>% 
  mutate(label = paste0("# died out = ", num_died)) %>% 
  mutate(x = Inf, y = Inf)

quadrant_plot <- quadrant_df %>% 
  filter(trajectory < 21) %>% 
  ggplot() +
...
ggsave("figures/example_quadrants_reduced.png", quadrant_plot, height = 20, width = 20)
