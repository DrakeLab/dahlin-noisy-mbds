...
if (do_calc) {
  ...
  summary_df = read_rds("./data/summary_data.rds", compress = 'gz')
  sims_out = read_rds("./data/some_sims.rds", compress = 'gz')
}
...
generic_plot_function <- function(output_name) {
  final_summary_df %>% 
    ...
    ggplot(aes(x = sigma, y = mean, group = R0_factor, color = R0_factor)) +
    # Mean
    geom_line(lwd = 1) +
    # Confidence intervals
    # geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, fill = R0_factor), alpha = 0) +
